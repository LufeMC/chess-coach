...
public struct Board: Sendable {
  ...
  /// that have appeared on this board, keyed by the position's Zobrist key.
  ///
  /// This is used to determine draw by repetition.
  ///
  /// - note: This fork keys on ``Position/zobristKey`` rather than
  /// `hashValue`. Swift's `Hasher` is seeded per process, so counts could not
  /// be persisted or compared across launches, and its 64-bit output was being
  /// truncated to `Int` for the key.
  private var positionKeyCounts: [UInt64: Int]
  ...
  ///
  /// Three or more is a threefold repetition draw. Exposed so the app can warn
  /// before a repetition rather than only reacting to ``Board/state``.
  public var repetitionCount: Int {
    positionKeyCounts[position.zobristKey, default: 0]
  }
  ...
  private mutating func updateState(after move: Move? = nil) {
    ...
    // draw by repetition
    positionKeyCounts[position.zobristKey, default: 0] += 1
    ...
    } else if positionKeyCounts[position.zobristKey, default: 0] >= 3 {
